...
const currentProductImg = document.querySelectorlector(".productImg");
...
menuItems.forEach((item,index) => {
  item.addEventListener("click",() => {
    ...
    currentProductTitle.textContent = choosemProduct.title;
    currentProductPrice.textContent = "RM" + choosenProduct.price;
    currentProductImg.src = choosenProduct.colors[0].img;

    //assing new colors
    currentProductColors.forEach((color,index) => {
      color.computedStyleMap.backgroundColor = choosenProduct.colors[index].code;
    });
});
});
